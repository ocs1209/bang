...
+ (void)addNetIndicator:(UIView*)container message:(NSString*)aMessage tagAndCancelTarget:(id)viewTagAndCancelTarget selector:(SEL)cancelSelector {
    ...
    BOOL hasMessage = !(aMessage == nil || [aMessage isEqualToString:@""]) ? YES : NO;
    ...
    if (hasMessage || cancelSelector) {
        ...
        if (cancelSelector) {
            ...
            cancelButton.frame = CGRectMake(labelRect.origin.x, labelRect.origin.y + labelRect.size.height + 12, labelRect.size.width, 44);
