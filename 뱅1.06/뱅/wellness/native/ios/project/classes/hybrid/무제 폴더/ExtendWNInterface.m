...
//#define GATEWAY @"http://gateway.rebon.co.kr/api/fit/sync/summary/send"
//#define GATEWAY @"http://192.168.21.15:8080/api"
//#define GATEWAY @"http://192.168.21.25:8080/api"
...
- (NSString *) exWN2PluginFitBandHistory : (NSString *) userKey : (NSString *) licenseKey : (NSString *) sleepStartTime : (NSString *) sleepEndTime : (NSString *) isLogin {
...
- (NSString *) PluginFitBandHistory : (NSString *) lastUploadDate : (NSString *) userKey : (NSString *) licenseKey : (NSString *) sleepStartTime : (NSString *) sleepEndTime {
    ...
    if (maxDate == nil) {
        ...
        // 일주일 날짜 넣기
        for (int i = 0; i < 7; i++) {
            todayComponents = [[NSCalendar currentCalendar] components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour fromDate:today];
            coreDate = [NSString stringWithFormat:@"%02d-%02d-%02d", (int)todayComponents.year, (int)todayComponents.month, (int)todayComponents.day];
            
            [arrayList addObject:coreDate];
            
            today = [today dateByAddingTimeInterval:60*60*24*-1];
        }
...
//            
//            AppDelegate *appDelegate =  (AppDelegate *)[[UIApplication sharedApplication] delegate];
//            NSManagedObjectContext *coreDataDeleteContext = [appDelegate managedObjectContext];
//            NSFetchRequest *delete_request = [[NSFetchRequest alloc] initWithEntityName:@"History_Sample"];
//            NSBatchDeleteRequest *delete_fetch_request = [[NSBatchDeleteRequest alloc] initWithFetchRequest:delete_request];
//            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"step_date >= %@", maxDate];
//            [delete_request setPredicate:predicate];
//            NSError *error = nil;
//            [coreDataDeleteContext executeRequest:delete_fetch_request error:&error];
//            
//            NSLog(@"%ld", (long)[self compareFromDateToDate:coreDateMaxDate:basicDate]); // from 이 더커지면 마이너스 됨
...
// 맥스 일자 가져오기 (서버기준)
//- (NSString *) getMaxDate : (NSString *) userKey {
//    
//    dispatch_async(dispatch_get_global_queue(0, 0), ^{
//        
//        NSString *request_url = LAST_SEND;
...
//        [self delete_history_core_data];
    ...
    NSString *postDataString = [NSString stringWithFormat:@"{\"head\" : {}, \"body\" : {\"auth_token\" : \"%@\", \"user_key\" : \"%@\", \"device\" : \"B\", \"items\" : [%@,%@]}}", authKey, userKey, stepJsonResult, sleepJsonResult];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            WriteBand *bandInit = [[WriteBand alloc] init];
...
- (NSString *) stepJsonBuild : (NSString *) jsonString arrayListDate:(NSArray *)arrayListDate userKey:(NSString *)userKey authKey:(NSString *)authKey sleepStartTime:(NSString *)sleepStartTime sleepEndTime:(NSString *)sleepEndTime {
    ...
    for (NSString *queryDate in arrayListDate) {
        ...
        NSString *jsonResult = [NSString stringWithFormat:@"{\"sync_date\" : \"%@\", \"datetime\" : \"%@\", \"type\" : \"STEP\", \"count\" : %ld, \"value\" : %ld, \"average\" : 0, \"device\" : \"B\", \"soso_count\" : 0, \"bad_count\" : 0, \"good_count\" : 0, \"soso_value\" : 0, \"bad_value\" : 0, \"good_value\" : 0, \"sleep_start\" : \"%@\", \"sleep_end\" : \"%@\"}", queryDate, queryDate, (long)stepCount, (long)totalStepValue, sleepStartTime, sleepEndTime];
...
- (NSString *) sleepBasicJsonBuild : (NSString *) queryDate sleepStartTime : (NSString *) sleepStartTime sleepEndTime : (NSString *) sleepEndTime {
    ...
    NSString *jsonResult = [NSString stringWithFormat:@"{\"sync_date\" : \"%@\", \"datetime\" : \"%@\", \"type\" : \"SLEEP\", \"count\" : 0, \"value\" : 0, \"average\" : 0, \"device\" : \"B\", \"soso_count\" : 0, \"bad_count\" : 0, \"good_count\" : 0, \"soso_value\" : 0, \"bad_value\" : 0, \"good_value\" : 0, \"sleep_start\" : \"%@\", \"sleep_end\" : \"%@\"}", queryDate, queryDate, sleepStartTime, sleepEndTime];
...
- (NSString *) ex2PluginFitAlarmSet : (NSString *) jsonString {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        NSString *enabled = [NSString stringWithFormat:@"%@", options[@"enabled"]];
        enabled = [enabled isEqualToString:@"true"] ? @"1" : @"0";
