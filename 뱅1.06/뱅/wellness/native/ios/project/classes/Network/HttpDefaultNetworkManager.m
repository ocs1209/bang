...
- (void)processWithViewCtrl:(UIViewController *)viewController trcode:(NSString *)path data:(NSString *)data networkoption:(PPNetworkOption *)networkOption delegate:(id<PPNetworkManagerDelegate>)delegate userinfo:(NSArray *)userInfo {
    ...
    if ( [url rangeOfString:@"http://localhost/"].location != NSNotFound ) {
        MPAppManifestConfig *manifestConfig = [MPAppManifestConfig getInstance];
        url = [url stringByReplacingOccurrencesOfString:@"http://localhost/" withString:[NSString stringWithFormat:@"http://localhost:%@/", @(manifestConfig.httpPort)]];
        
        NSMutableArray *trcodeComponents = [NSMutableArray arrayWithArray:[path componentsSeparatedByString:@"/"]];
        
        if ( trcodeComponents.count > 0 && [[trcodeComponents firstObject] isEqualToString:@""] ) {
            [trcodeComponents removeObjectAtIndex:0];
        }
        
        path = [[trcodeComponents componentsJoinedByString:@"."] stringByAppendingString:@".json"];
        
        networkOption.restAction = @"GET";
    }
    ...
    // Indicator 설정
    if ( [path rangeOfString:self.resourceUpdateURI].location == NSNotFound && networkOption.indicator ) {
        NSString *indicatorMessage = [NSString isEmpty:networkOption.indicatorMsg] ?
                                            @"데이터 송수신 중입니다.\n잠시만 기다려주세요." :  networkOption.indicatorMsg;
