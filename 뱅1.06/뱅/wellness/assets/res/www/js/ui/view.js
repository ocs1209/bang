			...
			_bindEvents: function() {
			    ...
			    $("#myPage").on("click", function(){
			    	if(M.data.global("BLE_LOCK") == "LOCK"){
			    		alert("동기화 중에는 설정 페이지로 이동 할 수 없습니다.");
			    		return;
			    	}else {
				...
				*/
                ...
                $('[data-link][auto-link]').bind("click.autolink", function(e) {
					...
					*/
                    ...
                    if ( link === "user.mypage.html" ) {
                    	if ( FitManager.defaultManager().synchronizeManager().isSyncing() ) {
                    		var popupController = PopupController.sharedInstance();
							popupController.confirm(
		                        "동기화 중에는 목표 페이지로 이동 할 수 없습니다.", 
		                        ["확인"],
                    ...
                    if ( link === "support.help.html" ) {
                    	if ( FitManager.defaultManager().synchronizeManager().isSyncing() ) {
                    		var popupController = PopupController.sharedInstance();
							popupController.confirm(
		                        "동기화 중에는 도움말  페이지로 이동 할 수 없습니다.", 
		                        ["확인"],
		                        function( buttonIndex ) {
	...
	constructor: function() {
		...
		return {
			...
			confirm: function( message, buttons, callback, waitUntilDone  ) {
				...
				M.pop.alert( {
					"title": "",
					"waitUntilDone": waitUntilDone === false ? false : true,
					"message": message, 
