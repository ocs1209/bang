...
public class HttpDefaultNetworkManager extends AsyncHttpNetwork {
	...
	@Override
	public void initFoundationInfo(String url, String agent, int timeout, String encoding, String resourceUpdateUri) {
		if ( url.contains("localhost") ) {
			url = url.replace("localhost", "localhost:" + CommonLibHandler.getInstance().g_nPort );
		}
		
		super.initFoundationInfo(url, agent, timeout, encoding, resourceUpdateUri);
	}
	...
	/user/login
	/ split -> [ "", "user", "login" ]
	remove first object -> [ "user", "login" ]
	. join + .json = user.login.json
	 ...
	 */
	@Override
	public synchronized void requestData(String trcode, final DataHandler sb, final String otherInfos, final Object objSender, NetReqOptions netReqOptions) {
		...
		// for local data load
		/*String joinedTrCode = "";
		String[] splitData = trcode.split("/");
		for (int i=0; i<splitData.length; i++) {
			if (i == 0) continue; 
			if (i == splitData.length-1) {
				joinedTrCode += splitData[i] + ".json";
				break;
			}
			joinedTrCode += splitData[i] + "."; 
		}
		
		final String trCode =  "/" + joinedTrCode;
		netReqOptions.method = "GET";*/
		final String trCode = trcode;
...
//			    [requestHeaders setObject:json_type   forKey:@"Accept"  ];
	...
	public JSONObject setRequestHeader(String otherInfos, int packetSeq, NetReqOptions netReqOptions) {
		 ...
		 try {
			 ...
			 //
			 subHeadJson.put("device_uuid", commHandle.g_strDeviceID);
			 subHeadJson.put("os_type", "A");
...
//			 subHeadJson.put("callback_function", otherInfos);
			 ...
			 try {
				 ...
				 String action     = _userData.has("action") ?_userData.getString("action") : "";
				 subHeadJson.put("action"    , action);
				 
				 String service_id = _userData.has("service_id") ?_userData.getString("service_id") : "";
