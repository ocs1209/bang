...
@objc public protocol xDelegate {
    func didHeartrate(v:Int)
    func didsetp(v:Int)
}
@objc public enum HandlerType: Int {
    case READDQ = 0//当前步数
    case READRA//心率
    case READLS//历史
    case READDL//电量
...
public typealias IsConnectBracelet_s = (output:AnyObject) -> Void
...
public class SXInterface: NSObject {
    ...
    public var conFunc = IsConnectBracelet_s?()
    ...
    public func IsConnectBracelet(blockfunc:IsConnectBracelet_s?){
        conFunc = blockfunc
        BleInterface.getDataCenter().IsConnect() { (output) in
            self.conFunc!(output: output)
        }
    }
